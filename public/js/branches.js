(function webpackUniversalModuleDefinition(root, factory) {
    ...
    return (function () {
        ...
        $(function () {
            var dt_branch_table = $('.datatables-branches'),
                select2 = $('.select2'),
                branchView = baseUrl + 'admin/branch/',
            ...
            if (dt_branch_table.length) {
                var dt_branch = dt_branch_table.DataTable({
                    ...
                    columnDefs: [
                        ...
                        },
                        {
                            ...
                            render: function render(data, type, full, meta) {
                                ...
                                let branchId = full.id;

                                return `
                                    <label class="switch switch-primary">
                                        <input
                                            id="switch-state-${branchId}"
                                            type="checkbox"
                    ...
                    buttons: [
                        ...
                        {
                            text: '<i class="ti ti-plus me-0 me-sm-1"></i><span class="d-none d-sm-inline-block">Add New Branch</span>',
                            className: 'add-new btn btn-primary create-record',
                            enabled: hasPermission('branch.create')
                        }
                    ],
            ...
            $(document).on('click', '.delete-record', function () {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        $.ajax({
                            type: 'DELETE',
                            url: baseUrl + `admin/branch/${branch_id}`,
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            },
            ...
            $(document).on('click', '.create-record', function () {
                window.location.href = baseUrl + 'admin/branch/create';
			});
            // edit record
            $(document).on('click', '.edit-record', function () {
                var branch_id = $(this).data('id');
                window.location.href = baseUrl + 'admin/branch/' + branch_id + '/edit';
                ...
                // get data
                // $.get(''.concat(baseUrl, 'admin/branch/').concat(branch_id, '/edit'), function (data) {
